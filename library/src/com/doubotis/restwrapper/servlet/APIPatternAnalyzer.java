...
public class APIPatternAnalyzer
{
    ...
    public APIPatternAnalyzer(Set<Class<? extends RequestImplementation>> subTypes)
    {
        Iterator<Class<? extends RequestImplementation>> it = subTypes.iterator();
        while (it.hasNext())
        {
            PatternDescriptor pd = new PatternDescriptor();
            Class<? extends RequestImplementation> impl = it.next();
            
            REST rest = impl.getAnnotation(REST.class);
            if (rest == null)
                continue;
            
            pd.mClass = impl.getName();
            
            Class<?>[] interfaces = impl.getInterfaces();
            for (int i=0; i < interfaces.length; i++)
            {
                if (interfaces[i].equals(IGetHandler.class))
                    pd.mCommands.add("GET");
                else if (interfaces[i].equals(IPostHandler.class))
                    pd.mCommands.add("POST");
                else if (interfaces[i].equals(IPutHandler.class))
                    pd.mCommands.add("PUT");
                else if (interfaces[i].equals(IDeleteHandler.class))
                    pd.mCommands.add("DELETE");
            }
            
            pd.mPattern = Pattern.compile(rest.path());
